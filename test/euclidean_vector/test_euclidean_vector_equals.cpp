...
TEST_CASE("2 euclidean vectors with different components element-wise should not be equal") {
	SECTION("Unequal component at index 0") {
		auto const lhs = comp6771::euclidean_vector({5.3, 0.0, 2.3});
		auto const rhs = comp6771::euclidean_vector({3.5, 0.0, 2.3});
		CHECK(lhs != rhs);
	}
	...
	SECTION("All unequal components") {
		auto const lhs = comp6771::euclidean_vector({1, 2, 3, 4, 5, 6});
		auto const rhs = comp6771::euclidean_vector({2, 3, 4, 5, 6, 7});
		CHECK(lhs != rhs);
	}

	SECTION("Negated euclidean vector") {
		auto const lhs = comp6771::euclidean_vector({1, 2, 3, 4, 5, 6});
		auto const rhs = comp6771::euclidean_vector({-1, -2, -3, -4, -5, -6});
		CHECK(lhs != rhs);
	}
}