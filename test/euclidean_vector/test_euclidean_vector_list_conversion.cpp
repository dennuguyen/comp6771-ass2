...
TEST_CASE("List type conversion should convert euclidean vector to list") {
	SECTION("Empty euclidean vector") {
		...
		auto const lst = static_cast<std::list<double>>(euc_vec);
		CHECK(lst.empty() == true);
	}

	SECTION("Zero euclidean vector") {
		auto const euc_vec = comp6771::euclidean_vector({0.0, 0.0, 0.0, 0.0});
		auto const lst = static_cast<std::list<double>>(euc_vec);
	...
	SECTION("1 dimension euclidean vector") {
		auto const euc_vec = comp6771::euclidean_vector({1.0});
		auto const lst = static_cast<std::list<double>>(euc_vec);
	...
	SECTION("2 dimension euclidean vector") {
		auto const euc_vec = comp6771::euclidean_vector({-42.0, 21.3});
		auto const lst = static_cast<std::list<double>>(euc_vec);
	...
	SECTION("3 dimension euclidean vector") {
		auto const euc_vec = comp6771::euclidean_vector({-2.032, 8.3293, 12.429});
		auto const lst = static_cast<std::list<double>>(euc_vec);
